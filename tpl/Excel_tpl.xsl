...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <msxsl:script language="javascript" implements-prefix="foo">
    ...
    var numHoja = 0
	 ...
	 var ret="0000-00-00T00:00:00.000"
	 var cadena=rtrim(ltrim(str))
		...
		<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
		 ...
		 xmlns:html="http://www.w3.org/TR/REC-html40">
			...
			<xsl:when test="count(/xml/rs:data/z:row)>0">
				<xsl:apply-templates select="/xml/rs:data/z:row[position() mod 65534 = 1]" mode="hojas">
				</xsl:apply-templates>	
			</xsl:when>
			<xsl:otherwise>
				<xsl:apply-templates select="/xml/rs:data" mode="hojas">
				</xsl:apply-templates>	
			</xsl:otherwise>
			</xsl:choose>
		</Workbook>
	...
	<xsl:template match="*" mode="cuerpo">
  		<Row>
					...
					<xsl:when test="$tipo_dato = 'dateTime'">
						<Cell ss:StyleID="s24">
				            ...
				            <xsl:variable name="time">
				               <xsl:value-of select="substring($valor,12,8)" />
				            </xsl:variable>
				            <Data ss:Type="DateTime"><xsl:value-of  select="$date" />T<xsl:value-of  select="$time" />.000</Data>
							</xsl:if>
						</Cell>
					</xsl:when>
 ...
 <xsl:template mode="hojas"  match="*" > 	  
 	<xsl:variable name="hoja" select="position()"/> 	 	
			  <Worksheet>
		        <xsl:attribute name="ss:Name">		        	
					    hoja <xsl:value-of select="$hoja"/>
		        </xsl:attribute> 
						<Table  x:FullColumns="1"
						 x:FullRows="1" ss:DefaultColumnWidth="70">
			    			...
			    			<xsl:apply-templates mode="cuerpo" select=".|following-sibling::*[not(position() &gt; 65533)]">
			    				<xsl:sort select="@id_socio"/>
			    			</xsl:apply-templates>
						</Table>
