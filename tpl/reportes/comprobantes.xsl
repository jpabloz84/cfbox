...
<div class="hoja">
  <div style="font-size: 25px;font-weight: bold;margin-left: 419px;margin-top: 4px;position: absolute;">
      <div style="width: 100%">
      	<xsl:value-of select="/xml/rs:data/z:row[position()=1]/@tipo_comprobante"/>
      </div>
      <div style="width: 100%">
      	<xsl:choose>
  ...
  <table class="tblfooter">
    <thead>
      <tr>
      <th>CAE</th><th>&nbsp</th><th>NETO</th><th>IVA</th><th>TOTAL</th>
      </tr>
    </thead>
    <body>
      <tr>
      ...
      <td>&nbsp</td>
      <td>$ <xsl:value-of select="sum(//z:row/@importe_neto)"/> </td>
      <td>$ <xsl:value-of select="sum(//z:row/@importe_iva)"/> </td>
      <td>$ <xsl:value-of select="sum(//z:row/@importe_item)"/> </td>      
      </tr>
      <tr>
	...
	<xsl:variable name="pos" select="position()"/>
		<tr>
	      ...
	      <td style="text-align: right;">$ <xsl:value-of select="@importe_tipo"/></td>
