        ...
        <table class="tbldetalle">          
            <thead>          
              <tr>
                ...
                <th style="width:8%;text-align: right">cant. ant.</th>
                <th style="width:8%;text-align: right">cant.</th>
                <th style="width:9%;text-align: right;padding-right: 3px">stock</th>
              </tr>
            </thead>
            <tbody>
    ...
    <xsl:param name="totalreg" />
     <xsl:for-each select="//z:row[((position() &gt; $filaini) and (position() &lt;= ($totalreg+$filaini)))]"> 
      <tr>
        ...
        <td style="text-align: right;">$ <xsl:value-of select="format-number(@precio_costo,'#.00')"/></td>
        <td style="text-align: right;">$ <xsl:value-of select="format-number(@precio_base,'#.00')"/></td>
        <td style="text-align: right;">$ <xsl:value-of select="format-number(@precio_iva,'#.00')"/></td>
        <td style="text-align: right;">$ <xsl:value-of select="format-number(@precio_venta,'#.00')"/></td>
