...
public $ID_ENVIO;
private $db;
//Parametro1:Elijo el servicio entre TESTING o PRODUCCION;parametro2:CUIT del emisor de comprobantes
//Parametro3:Nombre del certificado;parametro4:nombre del archivo de la clave privada;parametro5:clave del certificado
//Paramtro 6=es el path de la carpeta donde estan ubicados los archivos, y donde se van a generar los XMl temporales
//de no pasar nada, va a tomar como defecto, el path del archivo PHP que invoco la clase
function __construct($params){
    //para linux (sacar para windows)
   $pathCarpeta=str_replace("\\","/", $params['carpeta']);
   ...
   try {
    ...
    $this->WSDL_wsfe=$pathCarpeta."wsfev1Homo.wsdl";
    }else
    {
    ...
    $this->WSDL_wsfe=$pathCarpeta."wsfev1.wsdl"; 
    }
    $this->pathCarpeta=$params['carpeta'];
    ...
    //para windows
    //$pathcerts=str_replace("afip\\", "",$pathCarpeta);
    //para linuex
    $pathcerts=str_replace("afip/", "",$pathCarpeta);
    //para windows
    /*$this->certificado=$pathcerts.str_replace("/", "\\",$params['path_cer']);
    $this->clave_privada=$pathcerts.str_replace("/", "\\",$params['path_key']); */
    //para linux
    $this->certificado=$pathcerts.$params['path_cer'];
    $this->clave_privada=$pathcerts.$params['path_key'];
...
if (!file_put_contents( $pathfoldertmp."TA.xml", $TA)) 
{
$this->numError=3;
$this->descError=" No se pudo generar el ticket de acceso";
$evento['evento']='obtener_TA ->'.$this->numError;
$evento['detalle']=$this->descError;    
$this->regevent($evento);
return false;
...
try{
  $pathfoldertmp=str_replace("\\","/",$this->pathCarpeta);
  $file_ta=$pathfoldertmp."TA.xml";
  if (!file_exists($file_ta)) {
    $this->numError=4;
    $this->descError=" El TA no fue encontrado";
    $evento['evento']='TA_expirado ->'.$this->numError;
    $evento['detalle']=$this->descError;    
    $this->regevent($evento);
    return true;
  }

  $TA=simplexml_load_file($file_ta);
  $header=$TA->header;
  //echo "Fecha expiracion:";
  $expirationTime=$header->expirationTime;
  $f=new DateTime($expirationTime);
  $fecha_expiracion=$f->format(DATE_ISO8601);
  //si la fecha de expiracion es menor a la actual, el ticket no se  ha vencido todavia
  if(date(DATE_ISO8601)<=$fecha_expiracion)
  {$this->descError="Va a expirar el ".$fecha_expiracion;
    $evento['evento']='TA_expirado';
    $evento['detalle']=$this->descError;    
    $this->regevent($evento);
  return false;
  }else
  {
    $this->numError= 5;   
    $this->descError="Expiro el ".$fecha_expiracion;
    $evento['evento']='TA_expirado ->'.$this->numError;
    $evento['detalle']=$this->descError;    
    $this->regevent($evento);
    return true;
...
//verifica si el TA esta vigente o no, y si pertenece a la razon social
function verificaTA()
{
$this->numError=0;
$this->descError="";
...
try{
  ...
  if (!file_exists($file_ta)) {
    $this->numError=13;
    $this->descError=" El TA no fue encontrado";
    $evento['evento']='verificaTA ->'.$this->numError;
    $evento['detalle']=$this->descError;    
    $this->regevent($evento);
    return false;
  }
  ...
  $TA=simplexml_load_file($file_ta);
  $header=$TA->header;
  $dato_facturador=$header->destination;
  $pos=strpos($dato_facturador, (string)$this->cuit);
  /*esta validacion se saca porque para esta implementacion (FUNDACION EMANUEL)
  El cuit del representante es difernte a la de la fundacion
  */
  /*if($pos===false)
  {
   $this->numError=14;
   $this->descError=" El TA no pertenece al cuit cargado.TA datos: $dato_facturador. Cuit :".$this->cuit;
   $evento['evento']='verificaTA ->'.$this->numError;
   $evento['detalle']=$this->descError;    
   $this->regevent($evento);
   return false; 
...
$this->descError="";
 try{
    ...
    if (!file_exists($file_ta)) {
      $this->numError=6;
      $this->descError=" El TA no fue encontrado ->.".$this->numError.". Ruta:$file_ta ";      
